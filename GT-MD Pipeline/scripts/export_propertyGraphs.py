...
# ─── Ensure edgelists subfolder exists ────────────────────────────────
EDGELIST_DIR = EXPORT_DIR / "edgelists"
...
# convert any Decimal types to native floats
data = to_native(data)
...
with open(nodes_csv, "w", newline="") as fp:
    ...
    for mid, G, props in all_graphs:
        props_json = json.dumps(to_native(props))
        for n, attrs in G.nodes(data=True):
            w.writerow([mid, n, attrs.get("element",""), props_json, json.dumps(to_native(attrs))])
...
with open(edges_csv, "w", newline="") as fp:
    ...
    for mid, G, props in all_graphs:
        ...
        for u, v, attrs in G.edges(data=True):
            w.writerow([mid, u, v, props_json, json.dumps(to_native(attrs))])

# ─── Edge-list dumps ─────────────────────────────────────────────────
print("⏳ Writing edgelists…")

def sanitize_filename(name):
    # replace any Windows-invalid filename chars with underscore
    return re.sub(r'[<>:"/\\|?*]', '_', name)

for mid, G, _ in all_graphs:
    safe_mid = sanitize_filename(str(mid))
    out = EDGELIST_DIR / f"{safe_mid}_edgelist.txt"
...
def graph_to_data(mid, G, props=None):
    ...
    # 3) Graph-level props
    if props:
        for k, v in to_native(props).items():
...
print(f"\n✅ export_propertyGraphs complete. [{dt:.1f}s]")
